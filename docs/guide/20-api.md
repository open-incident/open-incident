...
| `GET` / `POST /incidents/{ref}/investigation`             | The root cause analysis (RCA) of an incident — hypotheses with confidence, findings and the evidence they cite — and a request for a new assessment (`202`; `409 unavailable` when the workspace may not have one).                                          |
