...
1. A route matches an alert (or a responder escalates an incident) and names a path — one chosen on the route, or the one a `service` or `team` attribute leads to (the owner team's path, or the team's own), with a fallback.
