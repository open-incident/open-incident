...
export async function ExceptionsTab({
  ...
  const t = await getT();
  let groups: Awaited<ReturnType<typeof exceptionGroups>> = [];
  let error: string | null = null;
  try {
    groups = await exceptionGroups(tenantId, { service, filter });
  } catch (err) {
  ...
  if (groups.length === 0) {
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        >
          {t(filter ? "explorer.noMatch" : "telemetry.noExceptions")}
        </div>
      </div>
    );
  }
